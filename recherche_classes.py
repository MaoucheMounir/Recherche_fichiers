...
class Retrieval(ABC):
    ...
    def __init__(self, fichiers, keywords) -> None:
    ...
    def retrieve(self) -> set[str]:
        ...
        while redo:
            for query in self.keywords:
                ...
                for file in self.fichiers:
                    content = self.get_content(file)
                    if self.contains(content, query):
            ...
            if redo:=ask_user_bin("Autre requête ? (y/n):\n"):
                self.keywords = input('Donnez un ou des mots-clé "mot1,mot2,...":\n').split(sep=',')
        
        print(dict(results_queries))
        self.resultat = all_pertinents
        return all_pertinents

    def contains(self, content:str, query:str) -> bool:
    ...
    def ouvrir(self, fichiers:dict):
        global filetype

    def get_content(self, file):
        pass
    
class RetrievalTxt(Retrieval):
    def __init__(self, search_path, keywords):
        Retrieval.__init__(self, search_path, keywords)
...
class RetrievalPDF(Retrieval):
    def __init__(self, search_path, keywords):
        Retrieval.__init__(self, search_path, keywords)
        
    def ouvrir(self, fichiers):
...
if __name__ == "__main__":
    ...
    if filetype == ".txt":
        retriever = RetrievalTxt(fichiers, keywords) 
    elif filetype == ".pdf":
        retriever = RetrievalPDF(fichiers, keywords) 
        
    retriever.retrieve()
    
    ## Ouvrir les fichiers
    if len(retriever.resultat) > 0 and ask_user_bin("Ouvrir les fichiers ? (y/n):\n"):
        retriever.ouvrir(retriever.resultat)


## Ajouter verif path et explore dans la classe 
